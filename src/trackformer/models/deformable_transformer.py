...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        """修改处"""
        # self.position = nn.Embedding(500, 2)
        self.adapt_pos2d = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.ReLU(),
            nn.Linear(d_model, d_model),
        )
        self.track_offset = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.ReLU(),
            nn.Linear(d_model, d_model),
            nn.ReLU(),
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, targets=None):
        ...
        if self.two_stage:
            ...
            # tgt = tgt.sigmoid()
            # reference_points = self.position.weight.sigmoid() # 使用sigmoid函数进行激活，将值固定到[0,1]的范围
            # reference_points = reference_points.unsqueeze(0).expand(bs, -1, -1)
            """修改处"""
            query_embed = self.adapt_pos2d(pos2posemb2d(reference_points))
            ...
            if targets is not None and 'track_query_hs_embeds' in targets[0]:
                ...
                if prev_hs_embed.shape[1] != 0:
                    list_offset = []
                    offset = self.track_offset(prev_hs_embed)
                    for i in range(prev_hs_embed.shape[0]):
                        # shape = targets[i]['size']
                        # shape_1 = torch.flip(shape, [0])
                        # list_offset.append(offset[i] / shape_1) # 将其正则化至当前空间
                        list_offset.append(offset[i])
                    offset_1 = torch.stack([t for t in list_offset])
                    # offset_ = offset.sigmoid()
                    prev_boxes = prev_boxes[..., :2] + offset_1
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, padding_mask=None):
        # self attention
        src2 = self.self_attn(0, self.with_pos_embed(src, pos), reference_points, src, spatial_shapes, padding_mask)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    #     self.sampling_offsets_track = nn.Linear(d_model, n_heads * n_levels * n_points * 2)
    #     self.modulation_track = nn.Linear(d_model, n_heads * n_levels * n_points * 1)
    #
    #     self._reset_parameters()
    #
    # def _reset_parameters(self):
    #     constant_(self.sampling_offsets_track.weight.data, 0.)
    ...
    #         grid_init[:, :, i, :] *= i + 1
    #     with torch.no_grad():
    #         self.sampling_offsets_track.bias = nn.Parameter(grid_init.view(-1))
    #     """修改"""
    #     constant_(self.modulation_track.weight.data, 0.)
    #     constant_(self.modulation_track.bias.data, 0.)
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, src_padding_mask=None, query_attn_mask=None):
        ...
        # cross attention
        # if query_pos.shape[1] > 500:
        #     tgt2 = self.cross_attn(self.sampling_offsets_track((self.with_pos_embed(tgt, query_pos))[:,:-500,:]).view(query_pos.shape[0], query_pos.shape[1]-500, self.n_heads, self.n_levels, self.n_points, 2) \
        #                            * (torch.sigmoid(self.modulation_track((self.with_pos_embed(tgt, query_pos))[:,:-500,:]).view(query_pos.shape[0], query_pos.shape[1]-500, self.n_heads, self.n_levels, self.n_points, 1))),
        #                            self.with_pos_embed(tgt, query_pos),
        #                            reference_points,
        #                            src, src_spatial_shapes, src_padding_mask, query_attn_mask, mask=True)
        # else:
        #     tgt2 = self.cross_attn(0,
        #                            self.with_pos_embed(tgt, query_pos),
        #                            reference_points,
        #                            src, src_spatial_shapes, src_padding_mask, query_attn_mask, mask=True)
        tgt2 = self.cross_attn(0,self.with_pos_embed(tgt, query_pos),reference_points,src, src_spatial_shapes, src_padding_mask, query_attn_mask)
...
"""相对于边界框的坐标做一个位置编码"""
def pos2posemb2d(pos, num_pos_feats=144, temperature=10000): # reference_points = (2,500,2)
    scale = 2 * math.pi
    pos = pos * scale
